...
#Plot creation
phylo_plot <- plot_bar(phylo_desulfo_rarefied, fill = "Family") +
  facet_wrap(~conductivity_category, scales = "free_x") +
  theme_classic() +
  geom_bar(aes(fill = Family), stat = "identity", position = "stack", color = NA) +
  labs(title = "Relative Abundance of Microbial Families in Thermodesulfobacteriaceae Phylum",
...
ggsave("results/taxonomy_family_barplot.png", plot = phylo_plot, width = 10, height = 5, dpi = 300)
